...
- Битые data-строки TSV (короткая строка или нечисловые `Impressions/Clicks/Ctr`)
  ...
  колонки `Query` или пустое/состоящее только из пробелов значение `Query` в
  data-строке — безопасная structured HTTP 502, а не ложный пустой отчёт.
  Другие malformed data-строки сохраняют текущую стратегию отбрасывания.
