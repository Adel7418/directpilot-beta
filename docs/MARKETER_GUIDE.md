...
| Изменить только ценность существующей priority goal | `POST /yandex/campaigns/{campaign_id}/priority-goals` | Только для ручного поиска (`HIGHEST_POSITION` + `SERVING_OFF`). Сначала `dry_run=true`: endpoint читает текущую кампанию, показывает `before_value_rub → after_value_rub`, полный `campaigns.update` preview и сохранённые поля. Не создаёт goal и не переключает стратегию. Apply — только после явного подтверждения: `live_write` + `approved=true` + `idempotency_key` + `dry_run=false`; readback обязан подтвердить выбранную value, остальные goals, Search/Network, CounterIds и бюджет. `value_rub` — ценность цели в ₽, не фактический CPA. |
