...
## Live geo, groups and text ads

**Endpoint map:**

- `GET /yandex/campaigns/{campaign_id}/ad-groups` — текущие `RegionIds` и readback группы;
- `GET /yandex/regions/resolve?name=...` — авторитетное точное разрешение региона;
- `POST /yandex/campaigns/{campaign_id}/ad-groups` — создание новой группы;
- `POST /yandex/campaigns/{campaign_id}/ad-groups/{ad_group_id}/geo` — полная замена набора регионов группы;
- `POST /yandex/ad-groups/{ad_group_id}/ads` — добавление текстовых объявлений;
- `PATCH /yandex/ads/{ad_id}` — точечное обновление полей объявления.

- Маркетолог готовит только read-only и dry-run preview. Live-apply не выполняет.
- Обязательная последовательность: `readback` текущего состояния → точный резолвер региона → `dry-run` preview + diff → явное подтверждение пользователя → оператор/оркестратор применяет с `DIRECTPILOT_MODE=live_write`, `approved=true`, новым `idempotency_key`, `dry_run=false` → provider readback.
- Неясный/неоднозначный регион и неуверенный readback/preview = **fail-closed**: не продолжать изменений.

**Разделение сценариев:**

- `POST /yandex/campaigns/{campaign_id}/ad-groups` — это создание новой группы.
- `POST /yandex/campaigns/{campaign_id}/ad-groups/{ad_group_id}/geo` — только полная замена `RegionIds` существующей группы.
- Для geo-diff явно показывать добавленные/убранные регионы и риск: покрытие, потенциал переполнения/потери трафика и стоимости.

**Нельзя через geo-маршрут менять:**

- ключи, минус-слова, объявления, стратегию, бюджет, цели, links/UTM, assets.

**Ad/PATCH:**

- `PATCH /yandex/ads/{ad_id}` меняет только переданные поля.
- `Href`, `UTM`, `BusinessId`, `SitelinkSetId`, `PreferVCardOverBusiness` сохраняются, если пользователь явно не просит иное.
- Новые/обновлённые объявления могут уйти на модерацию.

**Расширение в пригород/зону обслуживания:**

- Нужно явное подтверждение зоны обслуживания со стороны бизнеса.
- Разделять сегменты по гео.
- Ключи и тексты должны быть правдивы для конкретного service-area.
- Отдельный UTM/reporting per-segment.
- Это не даёт права изменять остальные параметры кампании без отдельного согласования.

- Никогда не обходить DirectPilot через raw Yandex API.
