...
- Не применять `POST /yandex/campaigns/{campaign_id}/time-targeting` с `dry_run=false` без явного `approved=true` + `idempotency_key` и подтверждения пользователя. В `live_readonly` реальный apply заблокирован с HTTP 409 до любого сетевого вызова; реально применить можно только в `live_write`. Используйте `dry_run=true` для проверки payload preview перед apply. Эндпойнт автоматически читает текущий `DailyBudget` кампании; если дневной бюджет есть, включает его с нормализованным `Mode` в payload update — без этого Direct возвращает error 8000. Если чтение бюджета сломалось/неоднозначно, apply отклоняется с 502; `DailyBudget: null` означает, что дневного бюджета нет и сохранять нечего.
