...
from app.db.models import ExternalIdentityRecord, YandexProviderConnectionRecord
...
class _RecordingCredentialPersister:
    ...
    def persist_yandex_oauth_tokens(
        ...
        *,
        ...
        assert subject == "stable-yandex-user-id"
...
class _SuccessfulPersistenceFailureProvider:
    access_token = "synthetic-access-token-for-persistence-failure"
    refresh_token = "synthetic-refresh-token-for-persistence-failure"

    def __init__(self, config: YandexOAuthConfiguration) -> None:
        self._config = config

    def exchange_code(self, *, code: str, code_verifier: str) -> YandexOAuthTokenSet:
        assert code == "synthetic-callback-code-for-persistence-failure"
...
class _FailingCredentialPersister:
    upstream_body = "synthetic-upstream-persistence-body"
    ...
    def persist_yandex_oauth_tokens(
        ...
        *,
        ...
        assert payload.access_token
        self.calls += 1
        raise ProviderConnectionPersistenceError(self.upstream_body)


@pytest.mark.integration
def test_credential_persistence_failure_is_safe_and_keeps_session_and_connection_absent(
    postgres_service: object,
    caplog: pytest.LogCaptureFixture,
) -> None:
    owner_runtime = create_database_runtime(
    ...
    try:
        ...
        provider = _SuccessfulPersistenceFailureProvider(config)
        credential_persister = _FailingCredentialPersister()
        session_service = PostgresSessionService(app_runtime.sessions)
        dependencies = replace(
            create_application_dependencies(),
            repository=PostgresLegacyStoreRepository(app_runtime.sessions),
        ...
        login = client.post(
            "/api/v1/_test/identity/login",
            json={"display_name": "Synthetic persistence failure owner"},
        )
        assert login.status_code == 200
        ...
        start = client.get("/api/v1/integrations/yandex/start", follow_redirects=False)
        assert start.status_code == 302
        state = parse_qs(urlsplit(start.headers["location"]).query, strict_parsing=True)["state"][0]
        authorization_code = "synthetic-callback-code-for-persistence-failure"
        caplog.set_level(logging.INFO, logger="directpilot.request")
        ...
        assert "oauth_credential_persistence_failed" in callback.text
        assert callback.headers.get("location") is None
        assert callback.headers.get("set-cookie") is None
        current_session_token = client.cookies.get(SESSION_COOKIE_NAME)
        assert current_session_token is not None
        assert hmac.compare_digest(previous_session_token, current_session_token)
        assert session_service.authenticate(previous_session_token) is not None
        assert credential_persister.calls == 1
        with tenant_transaction(
            app_runtime.sessions,
            workspace_id=workspace_id,
        ...
        sensitive_markers = (
            ...
            authorization_code,
            credential_persister.upstream_body,
        )
        assert not _contains_any(callback.text, sensitive_markers)
        log_output = "\n".join(str(record.__dict__) for record in caplog.records)
        assert not _contains_any(log_output, sensitive_markers)
    finally:
        owner_runtime.close()
